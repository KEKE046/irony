mod hw_test {
    ...
    pub fn create() -> (CmtIR, OpId) {
        ...
        cmt.with_region(Some(module_body), |cmt| {
            ...
            cmt.add_op(
                CombUnary::new(Some(e), Some(d), Some(CombUnaryPredicate::Not)).into(),
            );

            cmt.add_op(
                CombICmp::new(Some(cond), Some(e), Some(d), Some(CombICmpPredicate::EQ))
                    .into(),
            );
            ...
            cmt.add_op(
                SeqCompReg::new(Some(h_reg), Some(h), Some(clk), None, None).into(),
            );
    ...
    pub fn print_test() -> Result<(), ()> {
        ...
        cmt.pass_manager.add_passes(vec![PassEnum::RenamePass(RenamePass)], vec![
            no_parent.to_owned(),
        ]);
